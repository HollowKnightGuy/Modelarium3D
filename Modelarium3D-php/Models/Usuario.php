...
class Usuario
{
	...
	public function validarDatosRegister($datos_usuario, $message): array|bool
	{
		...
		$nombreval = "/^[0-9a-zA-ZáéíóúàèìòùÀÈÌÒÙÁÉÍÓÚñÑüÜ\s]+$/";
		...
		else if(strlen($datos_usuario['nombre']) > 12){
			$message['nombre'] = "The name can only be 12 characters long";
		}else {
			$message['nombre'] = "";
		}
		...
		if (!preg_match($passwval, $datos_usuario['password'])) {
			$message['password'] = "The password must measure between 6 and 14 characters, at least have a number, a lower case and an upper case";
		} else {
			$message['password'] = "";
		}
